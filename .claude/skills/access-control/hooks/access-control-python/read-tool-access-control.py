...
# ///
"""
Claude Code Access Control - Read Hook
=======================================

Blocks reads from protected files via PreToolUse hook on Read and Grep tools.
Loads zeroAccessPaths from access-control-list.yaml.

readOnlyPaths allow reads — only zeroAccessPaths are fully blocked here.
...
def match_path(file_path: str, pattern: str) -> bool:
    ...
    if is_glob_pattern(pattern):
        ...
        expanded_pattern_lower = expanded_pattern.lower()
        if expanded_normalized_lower.startswith(expanded_pattern_lower) or \
           expanded_normalized_lower == expanded_pattern_lower.rstrip('/'):
            return True
        return False
...
def check_path(file_path: str, config: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    Only zeroAccessPaths are blocked — readOnlyPaths allow reads.
    """
...
def main() -> None:
    ...
    # Handle Read tool (file_path) and Grep tool (path)
    if tool_name == "Read":
